...
// Go has support for multiple return values. This feature is often used in
// idiomatic Go, for example to return both result and error values from a
// function.
...
// Here's a function that will take an arbitary number of ints as arguments.
func sum(nums ...int) {
	fmt.Print(nums, " ")
	total := 0
...
func main() {
	...
	// Here we use 2 different return values from the call with multiple
	// assignment.
	a, b := vals()
	fmt.Println(a)
	fmt.Println(b)

	// If you only want a subset of the returned values, use the blank id _.
	_, c := vals()
	fmt.Println(c)
