...
// A fundamental concept in net/http is handlers. A handler is an object
// implementing the http.Handler interface. A common way to write a handler is
// by using the http.HandlerFfunc adapter on functions with the appropriate
// signature.
...
func main() {
	...
	// Finally, we call the ListenAndServe with the port and a handler. nil
	// tells it to use the default router we've just set up.
