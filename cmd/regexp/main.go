...
func main() {
	...
	// Above we used a string pattern directly, but for other regexp tasks you
	// will need to Compile an optimized Regexp struct.
	r, _ := regexp.Compile("p([a-z]+)ch")

	// Many methds are available on these structs. Here's the same match test.
	...
	// MustCompile variation of Compile. MustCompile panics instead of returning
	// an error, wihch makes it safer to use for global variables.
	...
	// The Func variant allows you to transform matched text with a given func.
	in := []byte("a peach")
	out := r.ReplaceAllFunc(in, bytes.ToUpper)
	fmt.Println(string(out))

	// For a complete reference on Go regular expressions check the regexp
	// package docs:https://pkg.go.dev/regexp
}
